...
export const getCommonConfig = ({
  ...
  plugins?: {
    /**
     * 构建前清空输出目录，使用 clean-webpack-plugin
     */
    clean?: true | ConstructorParameters<typeof CleanWebpackPlugin>[0]
    /**
     * 使用 webpack.BannerPlugin 输出顶部注释
     */
    banner?: string | ConstructorParameters<typeof webpack.BannerPlugin>[0]
    /**
     * 使用 copy-webpack-plugin 拷贝指定文件或内容到指定目录
     */
    copy?: ConstructorParameters<typeof CopyWebpackPlugin>[0]
    /**
     * 使用 html-webpack-plugin 生成入口 html，并将脚本注入其中
     */
    html?: boolean | ConstructorParameters<typeof HtmlWebpackPlugin>[0]
  }
  /**
  ...
  return {
    ...
    devServer: getDevServerConfig({
      open: plugins.html !== false
    }),
    /**
     * entry: string | string[] | {<entryChunkName>: string | string[]}
     ...
     * @see https://webpack.docschina.org/concepts/entry-points/
     * @example
     *   entry: './src/index.ts',
    ...
    output: {
       ...
       * @default dist
       */
      path: path.resolve(process.cwd(), name ? `dist-${name}` : 'dist')
    },
    resolve: {
